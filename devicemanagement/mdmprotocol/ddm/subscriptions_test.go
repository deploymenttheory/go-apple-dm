...
func TestSubscriptionActivationLifecycle(t *testing.T) {
	...
	if len(snap.Items) != 2 {
		t.Fatalf("automatic subscription has no activation: %+v", snap.Items)
	}
	// A new engine must serve the exact activation advertised by persistent state.
	restarted, err := ddm.New(ddm.Config{Store: h.store, Subscriptions: ddm.Subscriptions{Enabled: true}, Clock: h.clock})
	if err != nil {
		t.Fatal(err)
	}
	body, err := restarted.Declaration(ctx, dev, schemaddm.KindActivation, ddm.SubscriptionActivationIdentifier)
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := restarted.Declaration(ctx, dev, schemaddm.KindConfiguration, ddm.SubscriptionActivationIdentifier); !errors.Is(err, ddm.ErrNotFound) {
		t.Fatalf("activation served under the wrong kind: %v", err)
	}
	if _, err := h.engine.Status(ctx, dev, capabilitiesReport(t, []string{"device.model.family"})); err != nil {
		t.Fatal(err)
	}
	updated, err := h.engine.Manifest(ctx, dev)
	if err != nil || updated.Items[0].ServerToken != snap.Items[0].ServerToken || updated.Items[1].ServerToken == snap.Items[1].ServerToken {
		t.Fatalf("capability update changed activation or failed to update subscription: %+v, %v", updated, err)
	}
	// Explicit activation policy wins and cannot be replaced by cached generated bytes.
	admin := h.put(activationWithPredicate(ddm.SubscriptionActivationIdentifier, "FALSEPREDICATE", ddm.SubscriptionIdentifier))
	h.assign(dev, admin.Identifier)
	if _, err := h.engine.Manifest(ctx, dev); err != nil {
		t.Fatal(err)
	}
	body, err = h.engine.Declaration(ctx, dev, schemaddm.KindActivation, admin.Identifier)
	if err != nil || decode(t, body)["ServerToken"] != admin.ServerToken {
		t.Fatalf("administrator activation lost: %s, %v", body, err)
	}
	if err := h.engine.DeleteDeclaration(ctx, admin.Identifier); err != nil {
		t.Fatal(err)
	}
	body, err = h.engine.Declaration(ctx, dev, schemaddm.KindActivation, admin.Identifier)
	if err != nil || decode(t, body)["ServerToken"] != snap.Items[0].ServerToken {
		t.Fatalf("deleted override did not return to generated activation: %s, %v", body, err)
	}
	disabled, err := ddm.New(ddm.Config{Store: h.store, Clock: h.clock})
	if err != nil {
		t.Fatal(err)
	}
	if _, err := disabled.Declaration(ctx, dev, schemaddm.KindActivation, ddm.SubscriptionActivationIdentifier); !errors.Is(err, ddm.ErrNotFound) {
		t.Fatalf("disabled engine served cached generated activation: %v", err)
	}
	if snap, err := disabled.Manifest(ctx, dev); err != nil || len(snap.Items) != 0 {
		t.Fatalf("generated declarations survived disabling subscriptions: %+v, %v", snap, err)
	}
}
...
func TestSubscriptions(t *testing.T) {
	...
	t.Run("Disabled", func(t *testing.T) {
		...
		for _, it := range snap.Items {
			if it.Identifier == ddm.SubscriptionIdentifier || it.Identifier == ddm.SubscriptionActivationIdentifier {
	...
	t.Run("BaselineWhenUnknown", func(t *testing.T) {
		...
		if len(snap.Items) != 2 || snap.Items[0].Identifier != ddm.SubscriptionActivationIdentifier || snap.Items[1].Identifier != ddm.SubscriptionIdentifier || snap.Items[1].Kind != schemaddm.KindConfiguration {
		...
		if err != nil || parsed.ServerToken != snap.Items[1].ServerToken || decode(t, body)["ServerToken"] != parsed.ServerToken {
			t.Fatalf("token %+v %v vs %+v", parsed, err, snap.Items[1])
		}
	...
	t.Run("AppearsInManifestAndItems", func(t *testing.T) {
		...
		// Activation precedes configurations, then identifiers sort lexically.
		if len(snap.Items) != 3 || snap.Items[0].Identifier != ddm.SubscriptionActivationIdentifier || snap.Items[1].Identifier != ddm.SubscriptionIdentifier || snap.Items[2].Identifier != "com.example.cfg" {
			t.Fatalf("manifest %+v", snap.Items)
		}
		sub := snap.Items[1]
		got, _ := items(t, h, dev)
