...
func TestErrorsClassify(t *testing.T) {
	for name, tc := range map[string]struct {
		...
		"Reject":   {discovery.ErrReject, fault.PermissionDenied, fault.Device, "com.apple.well-known.failed"},
		"Router":   {discovery.ErrRouter, fault.Internal, fault.Operator, ""},
		"NotHTTPS": {discovery.ErrNotHTTPS, fault.Internal, fault.Operator, ""},
	} {
