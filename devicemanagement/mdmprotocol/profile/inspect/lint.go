...
func Inspect(data []byte, o Options) Report {
	...
	parsed, err := profile.Parse(plain, profile.ParseOptions{Resolve: resolvePayload})
	if err != nil {
		r.add(
			"PayloadContent",
			"error",
...
// Apple publishes two schemas with the same SSO payload type. Its built-in
// Kerberos extension has a fixed identifier; all other extensions use the
// generic schema, including the Platform SSO settings that need version checks.
func resolvePayload(typ string, keys map[string]any) profiles.Payload {
	if typ == profiles.PayloadTypeExtensibleSingleSignOn {
		if keys["ExtensionIdentifier"] == "com.apple.AppSSOKerberos.KerberosExtension" {
			return new(profiles.ExtensibleSingleSignOnKerberos)
		}
		return new(profiles.ExtensibleSingleSignOn)
	}
