...
type GetTokenTokenParameters struct {
	// A security token to generate the server token. The `com.apple.watch.pairing` service
	// type requires this key.
	SecurityToken *string `plist:"SecurityToken,omitempty" json:"SecurityToken,omitempty"`
	// The identifier of the phone paired to the watch. The `com.apple.watch.pairing` service
	// type requires this key.
	PhoneUDID *string `plist:"PhoneUDID,omitempty" json:"PhoneUDID,omitempty"`
	// The identifier of the watch paired to the phone. The `com.apple.watch.pairing` service
	// type requires this key.
...
type ReturnToServiceResponseReturnToService struct {
	...
	// The Wi-Fi profile that installs after erasure when using return to service. The device
	// requires this when it doesn't have Ethernet access.
	...
	// the device uses this profile directly instead of fetching it from the server. The device
	// requires this key if its Automated Device Enrollment profile contains the
	// `configuration-web-url` key.
	MDMProfileData []byte `plist:"MDMProfileData,omitempty" json:"MDMProfileData,omitempty"`
	// The system uses the bootstrap token for return to service with app preservation. The
	// device requires this key when Automated Device Enrollment enables return to service for
	// the device.
	BootstrapToken []byte `plist:"BootstrapToken,omitempty" json:"BootstrapToken,omitempty"`
