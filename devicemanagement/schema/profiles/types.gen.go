...
type ActiveDirectoryCertificate struct {
	...
	// The name of the certificate authority (CA), which the device determines from the common
	// name (CN) of the Active Directory entry. Valid values:
	...
	// method. Omit this key for computer certificates.
...
//
// ContentCaching corresponds to mdm/profiles/com.apple.AssetCache.managed.yaml (Content Caching Service).
type ContentCaching struct {
	...
	// notifications in the upper corner of the screen. The device automatically displayed
	// alerts starting in macOS 10.13. In macOS 10.15 the alerts are off by default, but still
	// available through this setting.
...
//
// MobileAccounts corresponds to mdm/profiles/com.apple.MCX(Mobility).yaml (Mobile Accounts).
...
// SchemaPath returns the Apple schema file this type was generated from.
func (*MobileAccounts) SchemaPath() string { return "mdm/profiles/com.apple.MCX(Mobility).yaml" }
...
type ShareKit struct {
	...
	// only if there's no `SHKAllowedShareServices` key.
...
// SoftwareUpdate: The payload that configures the software update policy. Removed: use the
// declarative management `com.apple.configuration.softwareupdate.settings` configuration.
//
// SoftwareUpdate corresponds to mdm/profiles/com.apple.SoftwareUpdate.yaml (Software Update).
type SoftwareUpdate struct {
	// The URL of the software update catalog. This property isn't supported in macOS 11 and
	// later.
...
type PrivacyPreferencesPolicyControlServices struct {
	...
	// A system microphone. A profile can't grant access to the microphone; it can only deny
	// it.
	Microphone []PrivacyPreferencesPolicyControlIdentity `plist:"Microphone,omitempty" json:"Microphone,omitempty"`
	// Specifies the policies for the app via the Accessibility subsystem. In macOS 27.0, the
	// device shows a non-blocking notification for each application when this setting is
	// applied, and it allows the user to make changes to the setting in the System Settings
	// app.
	...
	// and HID events from all processes. A profile can't grant access to these events; it can
	// only deny it.
	ListenEvent []PrivacyPreferencesPolicyControlIdentity `plist:"ListenEvent,omitempty" json:"ListenEvent,omitempty"`
	// Allows the application to capture (read) the contents of the system display. A profile
	// can't grant access to the contents; it can only deny it.
...
type AirPrintAirPrint struct {
	...
	Port *int64 `plist:"Port,omitempty" json:"Port,omitempty"`
	// If `true`, Transport Layer Security (TLS) secures AirPrint connections. Available only
	// in iOS 11 and later.
...
type ParentalControlsApplicationRestrictions struct {
	...
	// The allow list of app item dictionaries. This property is deprecated in macOS 10.15 and
	// later - use `allowList` instead.
	...
	// later - use `pathAllowList` instead.
	PathWhiteList []string `plist:"pathWhiteList,omitempty" json:"pathWhiteList,omitempty"`
	// The allow list of app item dictionaries.
	AllowList []ParentalControlsApplicationRestrictionsAllowListAllowListItem `plist:"allowList,omitempty" json:"allowList,omitempty"`
...
// ParentalControlsApplicationRestrictionsAllowListAllowListItem: A dictionary defining an
// app for parental control.
type ParentalControlsApplicationRestrictionsAllowListAllowListItem struct {
	// The bundle ID of the app.
	BundleID string `plist:"bundleID" json:"bundleID"`
	// The identifier of the app. Obtain this value from the Security framework using
	// `SecCodeCopyDesignatedRequirement`.
	...
	Disabled *bool `plist:"disabled,omitempty" json:"disabled,omitempty"`
	// An array of nested helper applications.
	SubApps []ParentalControlsApplicationRestrictionsAllowListAllowListItem `plist:"subApps,omitempty" json:"subApps,omitempty"`
	// The name used for display purposes.
	DisplayName *string `plist:"displayName,omitempty" json:"displayName,omitempty"`
}
...
type Restrictions struct {
	...
	// If `false`, the system disables modifications of eSIMs. This also disables the phone
	// number sharing setup on iPhones, in iOS 27 and later.
	...
	// If `false`, disables Rosetta usage awareness. When Rosetta usage awareness is active,
	// the device displays a pop-up dialog to the user when launching an app that uses Rosetta.
	...
	// Settings. This restriction isn't supported on the user channel.
	...
	// This restriction isn't supported on the user channel.
	...
	// Deprecated: use the declarative management `com.apple.configuration.app.settings`
	// configuration.
...
type AppStore struct {
	...
	// If `true`, the system prevents App Store from launching. Restricts installations to
	// software updates only in macOS 10.10 through 10.13.
...
type AssociatedDomainsConfiguration struct {
	...
	// otherwise, the system ignores this value.
...
type DNSProxy struct {
	...
	// DNS proxy network extension. Use this to correctly identify the DNS proxy extension when
...
type Dock struct {
	...
	// One or more special folders that the device may create at user login time and place in
	// the Dock.
	...
	// user or migrated user logs in. This option has no effect for existing users. Only
	// available on the device channel.
	...
	// An array of items located on the Documents side of the Dock that users can't remove from
	// that location.
	StaticOthers []DockStaticItem `plist:"static-others,omitempty" json:"static-others,omitempty"`
	// An array of items located on the Applications side of the Dock that users can't remove
	// from that location.
	StaticApps []DockStaticItem `plist:"static-apps,omitempty" json:"static-apps,omitempty"`
	// An array of items located on the Applications side of the Dock that users can remove
	// from the Dock.
	PersistentApps []DockStaticItem `plist:"persistent-apps,omitempty" json:"persistent-apps,omitempty"`
	// An array of items located on the Documents side of the Dock that users can remove from
	// the Dock.
...
type ExchangeActiveSync struct {
	...
	// If `true`, the system enables S/MIME encryption for this account. This key is
...
type ExtensibleSingleSignOnKerberosExtensionData struct {
	...
	// use this to display help information or disclaimer text.
	...
	// domains, this calculation is automatic.
	...
	// password expiration to the user.
	...
	// is logged in with a mobile account.
	...
	// Kerberos extension uses this when checking password age after a change.
	...
	// traffic. Use this if the servers aren't discoverable through DNS. If you specify the
	// servers, the system uses them for both connectivity checks and attempts to use them
	// first for Kerberos traffic. If the servers don't respond, the device falls back to DNS
	// discovery. Format each entry the same as it would be in a `krb5.conf` file, for example:
	...
	// password changes, perform password sync, or retrieve the home directory.
	...
	// issuer. e.g "\*My CA2\*". If there's one remaining, it will be auto-selected. If there
	// more than one remaining, then the list is shorter.
...
type ExtensibleSingleSignOn struct {
	...
	ExtensionIdentifier string `plist:"ExtensionIdentifier" json:"ExtensionIdentifier"`
	// The team identifier of the app extension. The device requires this key on macOS and
	...
	// Extension also supports the method. Don't use this. Use the `AuthenticationMethod` key
	// in the `PlatformSSO` dictionary instead.
...
type ExtensibleSingleSignOnPlatformSSO struct {
	...
	// The amount of time (in seconds) after receiving or updating a `FileVaultPolicy`,
	// `LoginPolicy`, or `UnlockPolicy` that the system can use unregistered local accounts.
	// Required when `AllowAuthenticationGracePeriod` is set.
	...
	// If `true`, the system detects the password during web login and synchronizes it to the
	// local account password for the user.
...
type FileProvider struct {
	...
	// documents synchronization in any app. This doesn't impact the ability for apps to
	...
	// ignored if `ManagementAllowsKnownFolderSyncing` is set to `false`. This setting doesn't
...
type Font struct {
	// The user-visible name for the font. The device replaces this field with the actual name
	// of the font after installation. Each payload must contain exactly one font file in
	// trueType (.ttf) or OpenType (.otf) format. The device doesn't support collection formats
	// (.ttc or .otc).
...
type HomeScreenLayoutIconItem struct {
	...
	// `Folder`.
	DisplayName *string `plist:"DisplayName,omitempty" json:"DisplayName,omitempty"`
	// The bundle identifier of the app. The device requires this setting if the type is
	// `Application`.
	...
	// The URL of the existing web clip for this item. The device requires this setting if
	// `type` is `WebClip`. If more than one web clip exists with the same URL, the behavior is
	// undefined.
	URL *string `plist:"URL,omitempty" json:"URL,omitempty"`
}
...
type LoginWindow struct {
	...
	// If `true`, a local administrator user can bypass or disable managed preferences (MCX
	// settings) for their login session. The device presents the user with this option at
	// login only when the user is a local administrator, and other users are not logged in.
	AdminMayDisableMCX *bool `plist:"AdminMayDisableMCX,omitempty" json:"AdminMayDisableMCX,omitempty"`
	// If specified, allows a certain number of retries until the device shows a password hint.
	// The device shows no hints if set to a value of 0.
...
type MDM struct {
	...
	// SecurityInfoResponse.SecurityInfo. ADE-enrolled devices are automatically allowed to use
	// the bootstrap token for authentication.
...
type Screensaver struct {
	// If `true`, the device prompts the user for a password when the screen saver is unlocked
	// or stopped. When you use this prompt, you must also provide `askForPasswordDelay`.
	AskForPassword *bool `plist:"askForPassword,omitempty" json:"askForPassword,omitempty"`
	// The number of seconds to delay before the device requires a password to unlock or stop
	// the screen saver (the grace period). A value of `2147483647` (hexadecimal equivalent of
	// `0x7FFFFFFF`) disables this requirement, and a value of `0` immediately requires the
	// password. To use this option, you must set `askForPassword` to `true`.
...
type FDERecoveryKeyRedirection struct {
	// The URL to which the device sends FDE recovery keys instead of to Apple. The URL must
	// begin with https://.
	...
	// encrypt the recovery key when the device sends it to the redirected URL. The referenced
...
type SCEPPayloadContent struct {
	...
	// example.org. If a certificate authority has multiple CA certificates, use this field to
	...
	// The number of seconds to wait between subsequent retries. The device attempts the first
...
type SmartCard struct {
	...
	// unlocking. It's still allowed for other functions, such as signing emails and accessing
	// the web. The device requires a restart for a setting change to take effect.
	AllowSmartCard *bool `plist:"allowSmartCard,omitempty" json:"allowSmartCard,omitempty"`
...
type SystemExtensions struct {
	...
	// `NonRemovableFromUISystemExtensions` can't overlap.
...
type SystemPolicyRule struct {
	...
	// `PayloadDisplayName` or `PayloadDescription` into this field before adding the rule to
...
type SystemPreferences struct {
	...
	// The list of disabled System Settings extensions. The device enables all other items.
	// When you specify `DisabledSystemSettings`, the device ignores `DisabledPreferencePanes`
	// and `EnabledPreferencePanes`.
...
type VPNVPN struct {
	...
	ProviderBundleIdentifier *string `plist:"ProviderBundleIdentifier,omitempty" json:"ProviderBundleIdentifier,omitempty"`
	// If the VPN provider uses a system extension, this field is required. Not available in
	// watchOS.
	ProviderDesignatedRequirement *string `plist:"ProviderDesignatedRequirement,omitempty" json:"ProviderDesignatedRequirement,omitempty"`
	...
	// `ExcludeAPNs` and `ExcludeDeviceCommunication` properties. The system always excludes
	// the following traffic from the tunnel:
	...
	// If `1`, the device disables the Connect On Demand toggle in Settings for this
	// configuration. Not available in watchOS.
...
type VPNOnDemandRulesElementActionParameters struct {
	...
	// 200 HTTP status code, the device establishes a VPN connection in response. This key is
	// valid only if the value of 'DomainAction' is 'ConnectIfNeeded'.
...
type VPNIKEv2 struct {
	...
	// field to choose which provider to use for this configuration. If the VPN provider uses a
	// system extension, then this field is required.
	ProviderBundleIdentifier *string `plist:"ProviderBundleIdentifier,omitempty" json:"ProviderBundleIdentifier,omitempty"`
	// If the VPN provider uses a system extension, then this field is required.
	ProviderDesignatedRequirement *string `plist:"ProviderDesignatedRequirement,omitempty" json:"ProviderDesignatedRequirement,omitempty"`
	// If `AuthenticationMethod` is `SharedSecret`, the device uses this value for IKE
	// authentication.
	...
	// device rejects this profile payload if these requirements aren't met.
...
// VPNTransparentProxy: The dictionary to use when `VPNType` is `TransparentProxy`.
type VPNTransparentProxy struct {
	...
	// the device sends this certificate for IKE client authentication. If extended
	// authentication is used, this certificate can be used for EAP-TLS.
	PayloadCertificateUUID *string `plist:"PayloadCertificateUUID,omitempty" json:"PayloadCertificateUUID,omitempty"`
	// The password to use for the account credentials. Only used if `AuthenticationMethod` is
	// `Password`.
	Password *string `plist:"Password,omitempty" json:"Password,omitempty"`
...
type WebClip struct {
	...
	// property, install the profile through MDM.
...
// Required when `FilterURLs` is `true`.
...
type WiFi struct {
	...
	// service. The device places any network credentials into Class D storage within the
	// keychain and stores information about the network on disk in Class D.
...
type WiFiEAPClientConfiguration struct {
	...
	// the listed certificates. The device disables dynamic trust (the certificate dialogue) if
	// you specify this property without also enabling 'TLSAllowTrustExceptions'.
	...
	// 'TLSAllowTrustExceptions' with the value 'true'. If necessary, use a single "\*"
	// character to specify a wildcard for an individual component of the name, such as
	// 'wpa.\*.example.com'.
