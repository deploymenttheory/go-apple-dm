...
type AccountConfiguration struct {
	...
	// the local account that Setup Assistant creates is a managed account.
...
type InstallApplication struct {
	...
	// The URL of the app manifest, which needs to begin with `https:`. The server returns the
	// manifest as a property list that uses the `ManifestURL` format.
	ManifestURL *string `plist:"ManifestURL,omitempty" json:"ManifestURL,omitempty"`
	...
	// introduced in iOS 13, or any type of account driven enrollment. The only possible value
	// is:
	ChangeManagementState *string `plist:"ChangeManagementState,omitempty" json:"ChangeManagementState,omitempty"`
	// If `true`, install the app as a managed app. Otherwise, the system installs the app as
	// unmanaged. If you reinstall a manged app and omit this value or set it to `false`, the
...
// app, if you choose to provide it.
...
type InstalledApplicationListResponseInstalledApplicationList struct {
	...
	// response unless it's included in the `Items` request key.
	...
	// 26, and visionOS 26 it isn't present in the response unless it's included in the `Items`
	// request key.
	...
	// The source of the application. When Declarative Device Management manages the app, this
...
type ManagedApplicationListManagedApplicationItem struct {
	...
	// user. This code reports only once.
	...
	// If 'true', the app has feedback for the server. On macOS 11.3 and later, this value is
	// available if the device management server sent the request on the user channel.
...
// CancelEnhancedLogCollection: Cancel enhanced log collection on the device.
//
// CancelEnhancedLogCollection corresponds to mdm/commands/cancel.enhanced.log.collection.yaml (Cancel Enhanced Log Collection Command).
type CancelEnhancedLogCollection struct {
}

// SchemaPath returns the Apple schema file this type was generated from.
func (*CancelEnhancedLogCollection) SchemaPath() string {
	return "mdm/commands/cancel.enhanced.log.collection.yaml"
}

// CancelEnhancedLogCollectionResponse: Response to CancelEnhancedLogCollection.
//
// CancelEnhancedLogCollectionResponse corresponds to mdm/commands/cancel.enhanced.log.collection.yaml (Cancel Enhanced Log Collection Command).
type CancelEnhancedLogCollectionResponse struct {
}

// SchemaPath returns the Apple schema file this type was generated from.
func (*CancelEnhancedLogCollectionResponse) SchemaPath() string {
	return "mdm/commands/cancel.enhanced.log.collection.yaml"
}
...
type EraseDevice struct {
	...
	// Assistant. Prior to iOS 14, don't use this option with any other option.
...
// EraseDeviceReturnToService: The configuration settings for return to service.
...
type DeviceLock struct {
	...
	// iPad device. This value is available for a Mac with Apple silicon only.
...
type RestartDevice struct {
	...
	// only adds previously discovered kexts to the kernel cache.
...
type DeviceInformationResponseQueryResponses struct {
	...
	// level. Requires the Device Information access right.
	...
	// lights-out management (LOM) controller.
	...
	// of that user. Requires the Device Information access right.
	...
	// creates during ADE enrollment. Requires the Device Information access right.
	...
	// indicates that the device requires online authentication for every login.
	...
	// carriers. Requires the Network Information access right.
	...
	// The key to get an attestation of the device's properties. See the hardware requirements
	// for attestation below.
...
type DeviceInformationResponseQueryResponsesMDMOptions struct {
	...
	// enables Find My. Unsupervised devices ignore this value.
...
// “OSUpdateSettings-dictionary“. Requires the Device Information access right.
...
// control which updates appear in the Software Update pane in Settings.
...
// settable accessibility settings.
...
type SecurityInfoResponseSecurityInfo struct {
	...
	// doesn't apply to User-Enrolled devices.
	...
	// the key and wraps it as CMS data.
	...
	// secure operations to use the Bootstrap Token. The device automatically sets this value
	// if enrolled through Automated Device Enrollment (ADE). The user can also manually set
...
// status of the EFI firmware password.
...
type ProfileList struct {
	...
	// device ignores this key and always limits the results to managed profiles.
	ManagedOnly *bool `plist:"ManagedOnly,omitempty" json:"ManagedOnly,omitempty"`
}
...
type ProfileListResponseProfileList struct {
	...
	// If `true`, the current MDM service installed the profile. MDM can remove or replace all
	// profiles on supervised devices.
...
type SetAutoAdminPassword struct {
	...
	// the GUID of an administrator account that MDM created during Automated Device Enrollment
	// (ADE) enrollment, the command returns an error.
	GUID string `plist:"GUID" json:"GUID"`
...
// doesn't support user enrollment. Starting in iOS 16 and iPadOS 17, when setting the
// wallpaper for the first time, both locations update. After that, you can set either
// location separately.
...
type SettingsSettingsDataRoaming struct {
	...
	// data roaming. The device only applies this setting to the primary SIM.
	Enabled bool `plist:"Enabled" json:"Enabled"`
}
...
// requires the App Management access right, supports user enrollment. This setting fails for
...
// to apply to the app. Omit this setting to remove existing attributes.
...
// doesn't support user enrollment, and is available only on supervised devices.
...
// doesn't support user enrollment.
...
// organization operating the MDM server. This setting supports user enrollment.
...
// protocol. This setting doesn't support user enrollment.
...
type SettingsSettingsMDMOptionsMDMOptions struct {
	...
	// inactivity. The device sets this to `false` by default for a supervised enrollment.
	// Starting in iOS 26.6 and iPadOS 26.6, changing the effective value from reboot allowed
	// to reboot disallowed requires a reboot or a device unlock before the change takes
	// effect.
...
// available only on supervised devices and doesn't support user enrollment.
...
// settings. This setting doesn't support user enrollment.
...
// ScheduleOSUpdate: Schedule an update of the operating system on a device. Removed: use the
...
type ScheduleOSUpdate struct {
	...
	// ignores this command and returns an informational error if a Declarative Device
	// Management `SoftwareUpdateEnforcementSpecific` configuration manages the software
	// update, as the configuration takes precedence.
...
// OSUpdateStatus: Get the status of operating-system updates on a device. Removed: subscribe
...
type OSUpdateStatusResponse struct {
	...
	// the status for System Applications and Configuration Data updates when a Declarative
	// Device Management `SoftwareUpdateEnforcementSpecific` configuration manages a software
	// update.
...
// TriggerEnhancedLogCollection: Trigger enhanced log collection on the device.
//
// TriggerEnhancedLogCollection corresponds to mdm/commands/trigger.enhanced.log.collection.yaml (Trigger Enhanced Log Collection Command).
type TriggerEnhancedLogCollection struct {
	// The AppleCare token the device uses for authorizing the enhanced log collection session.
	AppleCareToken string `plist:"AppleCareToken" json:"AppleCareToken"`
}

// SchemaPath returns the Apple schema file this type was generated from.
func (*TriggerEnhancedLogCollection) SchemaPath() string {
	return "mdm/commands/trigger.enhanced.log.collection.yaml"
}

// TriggerEnhancedLogCollectionResponse: Response to TriggerEnhancedLogCollection.
//
// TriggerEnhancedLogCollectionResponse corresponds to mdm/commands/trigger.enhanced.log.collection.yaml (Trigger Enhanced Log Collection Command).
type TriggerEnhancedLogCollectionResponse struct {
}

// SchemaPath returns the Apple schema file this type was generated from.
func (*TriggerEnhancedLogCollectionResponse) SchemaPath() string {
	return "mdm/commands/trigger.enhanced.log.collection.yaml"
}
