...
// Package ddm holds the declarative device management declarations generated
// from Apple's device management schema: 64 schema files and 204 types.
//
// # Design
//
// Apple publishes the wire format of the declarative device management
// declarations as YAML in https://github.com/apple/device-management, pinned
// here alongside a historical release input for older-device contracts.
// Generating this package from those inputs derives the wire types,
// validation, and support metadata from Apple's schema (decision record
// 0003). Every type carries plist and json struct tags with Apple's wire
// keys, a Validate method driven by the schema's constraints, and support
// metadata queryable through Support(path) or the
