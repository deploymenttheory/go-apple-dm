...
func TestCertificateStatusPrecedesAllSideEffectsAndPinModes(t *testing.T) {
	for _, pin := range []service.PinMode{service.PinEnforce, service.PinWarn, service.PinOff} {
		t.Run(string(rune('0'+pin)), func(t *testing.T) {
			h := newHarness(t, service.Config{Pinning: pin, CertificateStatus: func(context.Context, *x509.Certificate) error { return revocation.ErrRevoked }})
			for _, ck := range []*mdm.Checkin{authenticate(t, "D"), tokenUpdate(t, "D", nil), checkinPlist(t, map[string]any{"MessageType": "DeclarativeManagement", "UDID": "D", "Endpoint": "tokens"})} {
				if _, err := h.core.Checkin(t.Context(), req(h.cert), ck); !errors.Is(err, revocation.ErrRevoked) || service.CodeOf(err) != service.CodeForbidden {
					t.Fatal(err)
				}
			}
			if _, err := h.core.Connect(t.Context(), req(h.cert), &mdm.Response{ID: deviceID("D"), Status: mdm.StatusIdle}); !errors.Is(err, revocation.ErrRevoked) {
				t.Fatal(err)
			}
			if len(h.events) != 0 {
				t.Fatal("revoked request emitted mutation events")
			}
			if _, err := h.store.Get(t.Context(), deviceID("D")); !errors.Is(err, storage.ErrNotFound) {
