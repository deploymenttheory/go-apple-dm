...
func ServerSchema(d sqlcommon.Dialect) ([]sqlcommon.MigrationSet, error) {
	...
	for _, get := range []func(sqlcommon.Dialect) (sqlcommon.MigrationSet, error){
		...
		ddmsql.MigrationSet, depsql.MigrationSet, inventorystore.MigrationSet, statestore.MigrationSet, eventstore.MigrationSet, webhook.MigrationSet, maintenance.MigrationSet,
	} {
		set, err := get(d)
		if err != nil {
			return nil, wrap(err)
