...
func TestScenarioRejectsIncorrectDeviceBehavior(t *testing.T) {
	for _, name := range []string{"command missing", "wake without command", "user command missing", "shared device command missing", "shared user command missing", "unauthenticated TokenUpdate accepted", "malformed challenge", "invalid digest challenge", "default erasure enabled", "escrow lost"} {
		t.Run(name, func(t *testing.T) {
			...
			if err := s.Execute(t.Context(), e, target.Simulator{}); err == nil {
...
// TestLabFailsWhenReadinessNeverArrives checks that the lab fails when readiness never arrives.
func TestLabFailsWhenReadinessNeverArrives(t *testing.T) {
	t.Parallel()
...
// TestLabRejectsMissingModuleScratchDirectory checks that the lab rejects a missing module
// scratch directory.
func TestLabRejectsMissingModuleScratchDirectory(t *testing.T) {
	missing := t.TempDir() + "/absent"
	for _, name := range []string{"TMPDIR", "TMP", "TEMP"} {
		t.Setenv(name, missing)
	}
	result := Run(t.Context(), &Environment{Instance: Instance{Mode: "simulated"}}, target.Simulator{}, Module{
