...
//
// Private lab documents use the same PascalCase convention as admin responses.
...
func address(ctx context.Context, listen string, retain bool) (string, net.Listener, error) {
	...
	if host != "127.0.0.1" && host != "localhost" && host != "::1" {
		return "", nil, fmt.Errorf("%w: lab listener must be loopback", errOperation)
	}
	if port != "0" && !retain {
...
func Start(ctx context.Context, w *Workspace, binary string, out io.Writer) (*Environment, error) {
	...
	if w.Adapter == AdapterDocker {
		// Containers bind their own listener; the host reaches the published port.
		if listener != nil {
			_ = listener.Close()
			listener = nil
		}
		addr = net.JoinHostPort("127.0.0.1", w.port())
	}
	...
	if base := w.PublicBase(); base != "" {
		env["DM_PUBLIC_URL"] = base
	}
	...
	if (w.Storage == "postgres" || w.Storage == "mysql") && w.DSN == "" {
		return nil, fmt.Errorf("%w: SQL workspace requires DSN in the private workspace document", errOperation)
	}
	...
	if w.Adapter == AdapterDocker {
		if err = e.startContainers(ctx, env, out); err != nil {
			return nil, err
		}
	} else if err = e.launch(ctx, env, binary, out, listener); err != nil {
		return nil, wrapError(err)
	}
	if err = e.ready(ctx, e.URL); err != nil {
		return nil, wrapError(err)
	}
	if err = e.authorizeLab(ctx); err != nil {
		return nil, err
	}
...
// launch starts the configured lab server process or embedded runtime.
func (e *Environment) launch(
	...
	// Do not inherit DM_* settings into an isolated lab environment.
...
// ready waits for the lab server.s readiness endpoint before running modules.
...
// Up supervises processes in the foreground. A separate dmctl lab down uses
...
func Up(ctx context.Context, w *Workspace, binary string, out io.Writer) error {
	lock, err := os.OpenFile(w.path("lab.lock"), os.O_CREATE|os.O_RDWR, 0o600)
	if err != nil {
		return wrapError(err)
	}
	...
	_, _ = fmt.Fprintln(out, "Lab ready:", e.URL, "mode="+w.Mode, "service=device-management")
	select {
	case <-stop:
		return nil
	case <-ctx.Done():
...
// Attach loads the recorded lab instance and its HTTP credentials from an existing
// workspace.
...
// Control executes the requested lifecycle control against the lab environment.
func (e *Environment) Control(
	...
	resp, err := c.Do(req)
	if err != nil {
		return nil, fmt.Errorf("%w: lab supervisor is unavailable", errOperation)
	}
...
func (e *Environment) oidcFixture(env map[string]string) error {
	...
	if !ok {
		return fmt.Errorf("lab transport %T does not support fixture trust", e.Client.Transport)
	}
...
func configureACMEIdentifierKey(env map[string]string) {
	if env["DM_ACME_HMAC_KEY"] == "" {
		// A separate derivation keeps issued identifiers usable after a lab restart.
