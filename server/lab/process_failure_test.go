...
func runLabChild() {
	...
	for range tick.C {
		...
		b, err := os.ReadFile(os.Getenv("LAB_EXIT_FILE"))
		if err == nil {
...
func TestSupervisorReportsUnexpectedProcessExit(t *testing.T) {
	for _, code := range []string{"0", "3"} {
		t.Run(code, func(t *testing.T) {
			...
			w.Settings = map[string]string{"LAB_CHILD": "1", "LAB_EXIT_FILE": exitFile}
...
func TestStartupFailureLeavesNoSupervisor(t *testing.T) {
	...
	for _, name := range []string{"lock directory", "invalid address", "descriptor directory", "cancelled", "split child exit"} {
		t.Run(name, func(t *testing.T) {
			...
			switch name {
			case "lock directory":
				if err := os.Mkdir(w.path("lab.lock"), 0o700); err != nil {
					t.Fatal(err)
				}
			...
			case "split child exit":
				...
				w.Settings = map[string]string{"LAB_CHILD": "exit"}
			}
