...
func TestWorkspaceRejectsIncompleteOrInvalidState(t *testing.T) {
	...
	for _, args := range [][2]string{{"unknown", "inmem"}, {"live", "unknown"}} {
		if err := Init(t.TempDir(), args[0], args[1], "127.0.0.1:0", AdapterProcess, nil); err == nil {
	...
	if err := Init(file, "live", "inmem", "127.0.0.1:0", AdapterProcess, nil); err == nil {
		t.Fatal("file replaced by workspace")
	}
	if err := initialize(file, nil); err == nil {
	...
	if err := Init(partial, "live", "inmem", "127.0.0.1:0", AdapterProcess, nil); err == nil {
		t.Fatal("partial identity accepted")
	}
	if err := initialize(filepath.Join(partial, "mdm"), nil); err == nil {
	...
	for _, data := range []string{"{", `{"Version":2}`, `{"Version":1,"Mode":"invalid","Topology":"all"}`} {
		writeFixture(t, filepath.Join(dir, WorkspaceFile), []byte(data))
		if _, err := Load(dir); err == nil {
...
// TestLabReportsRetainFailureAndBlockedStatus checks that lab reports retain failure and
// blocked status.
func TestLabReportsRetainFailureAndBlockedStatus(t *testing.T) {
	t.Parallel()
	e := &Environment{Instance: Instance{Mode: "live"}}
	results := []Result{}
	for _, err := range []error{errors.New("unavailable"), ErrBlocked} {
		result := Run(t.Context(), e, target.Simulator{}, Module{
			ID:    "test",
			Modes: []string{"live"},
			Steps: scenario(func(context.Context, *Environment, string) error { return err }),
		}, Options{Adapter: "test", Revision: "revision"})
	...
	for _, path := range []string{ResultsFile, JUnitFile, HTMLFile} {
		d := t.TempDir()
...
// TestLabHTTPAndControlFailures checks lab HTTP and control failures.
func TestLabHTTPAndControlFailures(t *testing.T) {
	t.Parallel()
...
func TestInitPreservesFileAtIdentityDirectory(t *testing.T) {
	...
	if err := Init(dir, "live", "inmem", "127.0.0.1:0", AdapterProcess, nil); err == nil {
...
// TestLoadReadsLegacyWorkspaceDocument checks that a workspace written before the rename
// still loads with its identities and settings.
func TestLoadReadsLegacyWorkspaceDocument(t *testing.T) {
	dir := t.TempDir()
	if err := Init(dir, "simulated", "inmem", "127.0.0.1:0", AdapterProcess, nil); err != nil {
		t.Fatal(err)
	}
	if err := os.Rename(filepath.Join(dir, WorkspaceFile), filepath.Join(dir, LegacyWorkspaceFile)); err != nil {
		t.Fatal(err)
	}
	w, err := Load(dir)
	if err != nil || w.Storage != "inmem" {
		t.Fatalf("legacy workspace: %+v %v", w, err)
	}
	if err = Init(dir, "simulated", "inmem", "127.0.0.1:0", AdapterProcess, nil); err == nil {
		t.Fatal("legacy workspace overwritten")
	}
}
