...
import (
	...
	"github.com/deploymenttheory/go-apple-dm/storage/ddm/inmem"
	"github.com/deploymenttheory/go-apple-dm/testpki"
)

var (
	recvKey = []byte("rrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrr")
	sendKey = []byte("ssssssssssssssssssssssssssssssss")
...
func mustHandler(t *testing.T, cfg proxyserver.Config) http.Handler {
	...
	cfg.ReplayStore = state.NewMemory()
	if cfg.RecvKey == nil {
		cfg.RecvKey = recvKey
	}
	if cfg.SendKey == nil {
...
func post(h http.Handler, body []byte, opts ...reqOpt) *httptest.ResponseRecorder {
	...
	if len(r.Header.Values(proxywire.HeaderSignature)) == 0 {
		sig, _ := proxywire.SignRequest(recvKey, r, body)
		r.Header.Set(proxywire.HeaderSignature, sig)
	}
...
func TestHandler(t *testing.T) {
	...
	if _, err := proxyserver.Handler(
		...
		err,
		proxyserver.ErrNoBackend,
	) {
		t.Fatalf("no backend: %v", err)
	}
	// The default logger is used when none is configured.
	h, err := proxyserver.Handler(
		proxyserver.Config{
			Backend:     newStub(),
			SendKey:     sendKey,
			RecvKey:     recvKey,
			ReplayStore: state.NewMemory(),
		},
	)
	if err != nil || h == nil {
...
func TestRoutes(t *testing.T) {
	...
	t.Run("WrongContentType415", func(t *testing.T) {
		...
		if w := post(
			h,
			dmPlist(t, nil),
			withHeader("Content-Type", proxywire.ContentType+"; charset=utf-8"),
		); w.Code != http.StatusOK {
...
func TestSignature(t *testing.T) {
	...
	if err := verifyResponse(w, sendKey); err != nil {
		t.Fatal(err)
	}
	for _, bad := range []string{"", "v1.invalid", proxywire.Sign(recvKey, body)} {
		w = post(h, body, withHeader(proxywire.HeaderSignature, bad))
		if w.Code != 401 {
			t.Fatal("invalid envelope accepted", w.Code)
		}
		if err := verifyResponse(w, sendKey); err != nil {
			t.Fatal(err)
		}
	}
	// The identical request cannot be applied twice, including through a second replica.
	st := state.NewMemory()
	...
	if got := post(b, body, withHeader(proxywire.HeaderSignature, signature)); got.Code != 401 {
		t.Fatal("replay accepted", got.Code)
	}
	for _, ep := range []string{"status", "declaration/configuration/missing", "failing"} {
		w = post(h, dmPlist(t, map[string]any{"Endpoint": ep}))
		if err := verifyResponse(w, sendKey); err != nil {
			t.Fatal(ep, err)
		}
	}
...
func verifyResponse(w *httptest.ResponseRecorder, key []byte) error {
	return proxywire.VerifyBoundResponse(
		key,
		w.Header().Get(proxywire.HeaderSignature),
		w.Result().Request.Header.Get(proxywire.HeaderSignature),
		w.Code,
...
func send(t *testing.T, c *http.Client, url string, body []byte, opts ...reqOpt) *http.Response {
	...
	sig, _ := proxywire.SignRequest(recvKey, req, body)
	req.Header.Set(proxywire.HeaderSignature, sig)
	for _, o := range opts {
		o(req)
	}
	resp, err := c.Do(req)
...
func TestAuth(t *testing.T) {
	...
	t.Run("Bearer", func(t *testing.T) {
		...
		h := mustHandler(
			t,
			proxyserver.Config{Backend: newStub(), Auth: proxyserver.BearerAuth("s3cret")},
		)
		if w := post(
			h,
			body,
			withHeader("Authorization", "Bearer s3cret"),
		); w.Code != http.StatusOK {
		...
		empty := mustHandler(
			t,
			proxyserver.Config{Backend: newStub(), Auth: proxyserver.BearerAuth("")},
		)
		if w := post(
			empty,
			body,
	...
	t.Run("MTLS", func(t *testing.T) {
		...
		if resp := send(
			t,
			tlsClient(t, srv, clientIdentity(t, ca, "mdm-role")),
			srv.URL,
			body,
		...
		if !handshakeFails(
			t,
			tlsClient(t, srv, clientIdentity(t, other, "impostor")),
			srv.URL,
			body,
		) {
		...
		if resp := send(
			t,
			tlsClient(t, lenient, clientIdentity(t, other, "impostor")),
			lenient.URL,
			body,
		...
		optional := tlsServer(
			t,
			h,
			&tls.Config{ClientAuth: tls.RequestClientCert, MinVersion: tls.VersionTLS12},
		)
		if resp := send(
			t,
			tlsClient(t, optional, clientIdentity(t, ca, "mdm-role")),
			optional.URL,
			body,
		); resp.StatusCode != http.StatusUnauthorized {
	...
	t.Run("Rejected", func(t *testing.T) {
		...
		h := mustHandler(
			...
			proxyserver.Config{
				Backend: newStub(),
				Auth:    func(*http.Request) error { return vetoed },
			},
		)
...
func TestCheckin(t *testing.T) {
	...
	t.Run("DeviceChannel", func(t *testing.T) {
		...
		w := post(
			h,
			dmPlist(t, map[string]any{"UDID": "DEV-1", "Endpoint": "status", "Data": data}),
		)
		if w.Code != http.StatusOK || w.Body.Len() != 0 {
			t.Fatalf("status: %d %s", w.Code, w.Body)
		}
		want := mdm.EnrollmentID{Channel: mdm.ChannelDevice, ID: "DEV-1"}
		if last := s.seen(); last.id != want || last.endpoint != "status" ||
			!bytes.Equal(last.data, data) {
	...
	t.Run("Malformed400", func(t *testing.T) {
		...
		tight := mustHandler(
			t,
			proxyserver.Config{Backend: newStub(), Decoder: plist.Decoder{MaxBytes: 16}},
		)
	...
	t.Run("TooLarge413", func(t *testing.T) {
		...
		if w := post(
			h,
			dmPlist(t, map[string]any{"Data": bytes.Repeat([]byte("x"), 64)}),
...
func TestBackendErrors(t *testing.T) {
	...
	t.Run("400", func(t *testing.T) {
		...
		for _, ep := range []string{"bogus", "big", "garbled"} {
			if w := post(
				h,
				dmPlist(t, map[string]any{"Endpoint": ep}),
...
func TestHandlerRequiresReplayStateAndIndependentKeys(t *testing.T) {
	for _, cfg := range []proxyserver.Config{
		...
		{Backend: newStub(), ReplayStore: state.NewMemory(), RecvKey: recvKey, SendKey: recvKey},
		{Backend: newStub(), ReplayStore: state.NewMemory(), RecvKey: []byte("short"), SendKey: sendKey},
	} {
		if _, err := proxyserver.Handler(cfg); !errors.Is(err, proxyserver.ErrUnauthorized) {
			t.Fatal(err)
		}
	}
	h, err := proxyserver.Handler(
