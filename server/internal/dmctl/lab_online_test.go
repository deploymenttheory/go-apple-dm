...
// TestLabCommandsUseMaintainedRuntime checks lab commands use maintained runtime.
func TestLabCommandsUseMaintainedRuntime(t *testing.T) {
	env := noConfig(t)
	dir := t.TempDir()
	if err := lab.Init(dir, "simulated", "inmem", "127.0.0.1:0", lab.AdapterProcess, nil); err != nil {
		t.Fatal(err)
	}
	w, err := lab.Load(dir)
	if err != nil {
	...
	go func() { done <- lab.Up(ctx, w, "", io.Discard) }()
	t.Cleanup(func() {
		cancel()
		select {
		case err := <-done:
	...
	call := func(args ...string) (string, error) {
		...
		out, _, err := run(t, env, append([]string{"lab"}, args...)...)
		return out, err
	}
	out, err := call("status")
	...
	if _, err := call("run", "-modules", "LIVE-001"); err == nil {
		t.Fatal("unsupported live scenario passed")
	}
	for _, args := range [][]string{{"replace"}, {"replace", "-device-id", "absent"}, {"unknown"}, {"profile"}, {"profile", "-file", filepath.Join(dir, "new.mobileconfig")}, {"run", "-modules", "no-such-scenario"}, {"up", "-dmserver", "missing"}, {"list", "extra"}, {"list", "-invalid"}} {
