...
| Stale DDM state after checkout or reset | Required completion clears associated declarative state and pending synchronization work, including dependent user channels for device transitions. Cleanup failure fails check-in; shared-SQL composition rolls back MDM, DDM and captured events together. | [Lifecycle completion](../../server/ddmsync/servicehook.go), [failure and rollback tests](../../server/ddmsync/lifecycle_integration_test.go) |
| Delayed Authenticate clears newer DDM assignments | Built-in stores classify reset or retry under their write lock; same-certificate retries preserve state. Legacy custom stores without the optional outcome use a pre-read fallback and require external serialization. | [Outcome contract](../../devicemanagement/storage/authenticate.go), [concurrent lifecycle tests](../../server/ddmsync/lifecycle_integration_test.go) |
| Administrative privilege escalation | Stored principals and scoped tokens use Cedar authorization against the canonical stored channel and parent, with immutable identity guards in all stores. Managed roles and schema-validated policies gate operations. The one-time bootstrap endpoint creates the first stored root; root can repair authorization but has no implicit fleet permissions. | [Admin authorization](../../server/adminauth/), [admin route enforcement](../../server/internal/app/adminauthz.go) |
...
DDM lifecycle completion across memory or independently committing stores has no
cross-store rollback or durable cleanup recovery guarantee. See
[decision 0020](../research/decisions/0020-ddm-engine-membership-and-storage.md)
for the shared-SQL guarantee and custom-store limits.
