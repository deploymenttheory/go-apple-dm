...
ddm/adapter/proxyserver NanoMDM/MicroMDM `-dm` compatible HTTP surface (X-Enrollment-ID, HMAC) so we are a drop-in DDM backend
ddm/adapter/proxyclient The inverse: we are the MDM, DDM lives elsewhere
...
acme/                   ACME subset with device-attest-01, attestation chain verification, nonce store, policy hooks
dep/                    Legacy DEP web service client (OAuth 1.0a, session, cursor sync, profiles)
...
| 4 | SQL backends | `sqlite`, `postgres`, `mysql`, `sqlcommon` migrations, contract suite in CI | nanomdm `storage/mysql`, `storage/pgsql`; kmfddm `storage/mysql` | n/a | Same contract on four backends; `Clear` on 100k rows under 1s on Postgres; pagination | Suites green on all backends; coverage gate | M |
| 5 | DDM engine | `ddm`, `ddm/adapter/*`, notifier, simulator DDM client | kmfddm `ddm/*`, `storage/*`, `http/api`; Fleet `server/service/apple_mdm.go` DDM handlers and `docs/Contributing/architecture/mdm/apple-declarative-device-management.md`; Zentral `zentral/contrib/mdm/declarations/protocol.py` | Declarations, Status items, DeclarativeManagementRequest, Integrating DDM; `declarative/**` | Canonical tokens; dynamic membership; status values retained; unenroll cleanup (#41); coalesced notifier (#11); NanoMDM `-dm` drop-in; predicates | e2e: change, push, tokens, items, fetch, status verified; predicate scenario; proxyserver interop against nanomdm container | L |
| 6 | Enrollment breadth | user and Shared iPad channels, account-driven discovery and auth flows, `dep`, DEP profile, MachineInfo, `axm` | nanodep `godep`, `client`; Zentral `dep.py`, `public_views/user.py`; Fleet `server/mdm/apple/apple_bm.go`; `vbnin/Apple-JSON-discovery-server`; `korylprince/dep-webview-oidc` | Device enrollment, Device assignment, account-driven pages; `other/machineinfo.yaml` | Cursor expiry handling; per-user-type discovery; typed MachineInfo | e2e: DEP profile assign (fake DEP), account-driven discovery, user channel commands | M |
| 7 | ACME and attestation | `acme`, attestation verification, policy hooks | `brandonweeks/nanoca` handlers; step-ca `acme/challenge.go` device-attest; `hslatman/ios-acme-simulator` | Identity management: Validating a Managed Device Attestation; `profiles/acmecertificate.yaml`, `assets/credential.acme` | Policy hooks, nonce store, persisted attestations | e2e: ACME enrol with simulated attestation; rejected chain | M |
