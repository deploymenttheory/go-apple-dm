...
| Decision | Choice |
|---|---|
| Deliverable | Library first, plus thin reference server `cmd/dmserver` and admin CLI `cmd/dmctl` |
...
> into tiers, and the module moved to `github.com/deploymenttheory/go-apple-dm`. Where this
...
- **Outputs** (all `*.gen.go`, header `// Code generated by admgen; DO NOT EDIT.`, matching the
  ...
  - `schema/support`: `Register(family, table)` and `Lookup(family, path) *Entry` over a compact
    ...
    the simulator (a fake iOS 17 device rejects keys it would not accept), and `dmctl explain`.
...
- **Naming contract**: Go identifier = Apple key with dots and hyphens removed, segments
  ...
  Rules live in `internal/schemagen/naming.go` with a table test. `schema/EXPORTED_IDENTIFIERS.lock` lists every
  exported identifier; `admgen verify` fails when one disappears without an entry in
  `schema/ALLOWED_REMOVALS.md`. Regeneration can add, never silently rename. This is the lesson from the
...
2026-09-02; it was split into phase 8 (admin API, authorization, `dmctl`) and phase 9
...
- Phase 8 ends with `dmctl` driving every admin route, a read-only principal refused and the
