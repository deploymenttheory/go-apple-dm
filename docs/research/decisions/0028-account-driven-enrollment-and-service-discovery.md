...
1. `enroll/discovery.Handler` serves `GET`/`HEAD /.well-known/com.apple.remotemanagement`: exact parsing of `model-family` and `user-identifier`, a `Router(ctx, ModelFamily, UserIdentifier) (Server, error)` hook returning `Version` (`mdm-byod` or `mdm-adde`) and an absolute `https` `BaseURL`, the response body exactly `{"Servers":[{"Version":...,"BaseURL":...}]}` with `Content-Type: application/json` and `Cache-Control: no-store`, 405 for other methods, and rejection as 403 `com.apple.well-known.failed` (JSON or plist by `Accept`), never a bare 404. A redirect helper re-attaches the query parameters Apple drops.
...
3. The first POST's signed plist (`LANGUAGE`, `PRODUCT`, `VERSION`, and the MachineInfo keys when present) is parsed and verified like ADE (0027), so the software update gate and identity selection run for account-driven enrollment too, and an unauthenticated retry for the same device does not mint a new challenge while one is pending. `enroll/accountdriven.Handler` implements both documented flows behind one `Authenticator` interface: `apple-as-web` (our web-auth page hook, `user-identifier` prefilled, `webauth` OIDC from 0027 or a custom page, 308 to `apple-remotemanagement-user-login://authentication-results?access-token=`) and `apple-oauth2` (we are the OAuth 2 authorization server for a public client: authorization endpoint, token endpoint with authorization code and refresh grants, `login_hint`, `state` echoed, `redirect-url` under the `apple-remotemanagement-user-login` scheme, 308 completion). The 401 header is built by one function whose parameters are validated (`https` only).
...
5. The BYOD and ADDE profiles come from the 0009 builder with the account-driven keys enforced: `EnrollmentMode` matches the discovery `Version` (`BYOD` or `ADDE`), `AssignedManagedAppleID` is always set (from the authenticated identity, never empty on renewal), `ServerCapabilities` includes `com.apple.mdm.user-enrollment`... as documented per mode, `PayloadScope` is `User` for BYOD, `AccessRights` follows Apple's user-enrollment limits, and `EnrollmentMode`/`AssignedManagedAppleID` are immutable across profile updates. macOS account-driven device enrollment is supported, not rejected.
...
3. `accountdriven.TestFirstPost/{BodyParsed,SoftwareUpdateGate,ChallengeIdempotent}`, `accountdriven.TestFlow/AppleAsWeb/{FirstPost401,HeaderParams,WebAuthPrefill,HandBack308,SecondPostProfile}`, `accountdriven.TestFlow/AppleOAuth2/{Header,AuthorizationCode,TokenEndpoint,LoginHint,StateEchoed,Refresh,BadCodeRejected}`, `accountdriven.TestHeader/HTTPSOnly`.
...
5. `enroll.TestAccountDrivenProfile/{BYOD,ADDE,ModeMatchesVersion,ManagedAppleIDRequired,ImmutableOnUpdate,MacADDESupported}` (macOS would fail on Fleet; empty ID on renewal would fail on Fleet).
