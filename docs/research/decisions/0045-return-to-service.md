...
When built with OS 27 seed-generated check-in types, the handler can set
...
supervision and ADE requirements. This is caller-selected policy; the stable
schema does not yet expose the field. The reference server does not enable retries
automatically.
...
The `schema_seed_os_27` service tests verify retry omission, false and true in the
