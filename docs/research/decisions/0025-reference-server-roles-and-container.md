...
2. `cmd/mdmserver` is a thin main (flags and `MDM_*` environment) with roles `mdm` (check-in and connect on `/mdm`; DDM through `inproc` when a local engine is configured or through `proxyclient` when `-ddm-url` is set), `ddm` (engine plus `proxyserver` on `/ddm/` plus the admin API), and `all` (both in one process through `inproc`).
