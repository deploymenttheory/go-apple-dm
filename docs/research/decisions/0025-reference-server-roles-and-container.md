...
2. `cmd/dmserver` is a thin main (flags and `DM_*` environment) with roles `mdm` (check-in and connect on `/mdm`; DDM through `inproc` when a local engine is configured or through `proxyclient` when `-ddm-url` is set), `ddm` (engine plus `proxyserver` on `/ddm/` plus the admin API), and `all` (both in one process through `inproc`).
