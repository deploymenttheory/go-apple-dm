...
Enrollment status values, errors and reports require `ReadEnrollmentStatus` and
return redacted projections scoped to the full enrollment identity. FileVault
escrow preparation requires `EnqueueCommand` for an enabled device enrollment;
its retained decryption identity has no administrative export route. See
[0021](0021-status-reports-and-subscriptions.md) and
[0054](0054-filevault-encryption-identities.md).
