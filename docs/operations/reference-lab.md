# Reference server APIs and lab configuration

These are ordinary reference-server capabilities. The lab supplies configuration
and exercises the same APIs as an operator. See the [runbook](../../test-lab/README.md)
for workspace commands and the [testing guide](../testing/lab.md) for test layers.
...
matches the device UUID. These options are inactive unless configured. The lab
...
| `POST /enrollment-links` | JSON `DeviceID` and the optional `EnrollmentProfileRequest` fields, plus `TTL` (Go duration, default `1h`, at most `24h`); returns 201 with `ID`, the token-bearing `URL` and `ExpiresAt`. The token is not stored or returned again. | `issueEnrollmentProfile` |
| `GET /enrollment-links` | Paged `Items` of link metadata with `State` (`active`, `redeemed`, `revoked`, `expired`), binding, `CreatedBy` and timestamps; never tokens. | `issueEnrollmentProfile` |
| `DELETE /enrollment-links/{id}` | Revokes an unredeemed link; repeating succeeds, a redeemed link returns 409. | `issueEnrollmentProfile` |
...
An enrollment link lets a device fetch its own enrollment profile from a browser
without an administrator credential. The public routes are:

| Route | Behavior |
|---|---|
| `GET /enroll/links/{token}` | HTML landing page naming the bound device, the expiry and links to the trust profile (when HTTPS trust anchors are configured) and the enrollment profile. Viewing it does not consume the link. |
| `GET /enroll/links/{token}/profile` | Atomically marks the link redeemed, then issues the bound profile through the same path as `POST /enrollment-profiles`, including admission policy and issuance metadata. Publishes `enrollment-link-redeemed`. |

A link issues at most one profile. Redemption is committed before issuance, so
concurrent requests cannot both receive a profile. If issuance then fails, the
link stays redeemed and the operator creates another. Unknown, expired, revoked
and redeemed tokens receive the same 404 response. Responses set `no-store`,
`Referrer-Policy: no-referrer` and a restrictive content security policy, because
the URL is a bearer credential. Link metadata is kept for 24 hours after expiry in
the transactional state table under the `enrollment-link:` namespace, keyed by the
token's SHA-256 digest.

`dmctl enrollment-links create -device-id UDID [-serial S] [-identity scep|acme]
[-access-rights N] [-scope System|User] [-ttl 30m]` prints the URL once.
`dmctl enrollment-links list` and `revoke -id ID` manage retained links.

## Administration tools

`dmctl apppush list|put|send` provides typed administration. `dmctl lab profile`
...
Use `dmctl lab` to supervise the reference runtime and execute scenarios.
...
one separately, while `dmctl lab replace` prepares and then requests the wake.
