...
|---|---|---|
| `POST /enrollment-profiles` | JSON `DeviceID`, optional `Serial` and `AccessRights`; returns a mobileconfig using the configured issuer and topic. Default rights request device inventory. | `issueEnrollmentProfile` |
