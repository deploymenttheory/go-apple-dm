...
envelopes are rejected. CMS envelopes may use BER indefinite lengths, as Apple's
native SCEP encoder does. After signature verification, the encryption-policy
check reads a bounded, definite-length view without changing the authenticated
bytes; decryption uses the original envelope. Truncated input, trailing data and
excessive nesting are rejected. Library clients require verified HTTPS for automatic CA
...
Replacement profiles use a 46-byte certificate common name. The macOS profile
installer truncates longer SCEP common names at 64 bytes, so the reference server
stores the device binding under `replacement-subject:<attempt UUID>` in persistent
protocol state instead of embedding the device ID in the subject. Retain these
records with the enrollment and certificate history when backing up or restoring
the server. They identify the device; the pending attempt, challenge, exact CSR
and certificate checks authorize the replacement. Existing legacy subjects
remain accepted for certificates already issued.
