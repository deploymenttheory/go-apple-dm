...
| `MDM_STORAGE_KEYS`, `MDM_STORAGE_KEY_<NAME>`, `MDM_SECRETS_DIR`, `MDM_STORAGE_KEYS_STRICT` | Keys sealing the secret columns of a durable store: unlock and bootstrap tokens, APNs push keys, user auth tokens. `MDM_STORAGE_KEYS` lists key names active-first, and the material comes from `MDM_STORAGE_KEY_<NAME>` or from files in `MDM_SECRETS_DIR`. A rotation prepends a name and runs `Rewrap`; `MDM_STORAGE_KEYS_STRICT` then refuses any row still in clear. A durable backend will not start without this |
