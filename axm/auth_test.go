...
func TestCredentials(t *testing.T) {
	...
	t.Run("SealedAtRest", func(t *testing.T) {
		...
		store, err := NewSealedStore(ring)
		if err != nil {
			t.Fatal(err)
		}
		key := newKey(t)
		pemBytes := sec1PEM(t, key)
		creds := Credentials{ClientID: testClientID, KeyID: testKeyID, PrivateKeyPEM: secrets.New(pemBytes)}
		...
		cfg, err := ConfigFrom(context.Background(), store, "prod")
	...
	t.Run("Failing", func(t *testing.T) {
		t.Parallel()
		if _, err := NewSealedStore(nil); !errors.Is(err, ErrStore) {
		...
		store, _ := NewSealedStore(ring)
		ctx := context.Background()
		if err := store.Put(ctx, "", Credentials{}); !errors.Is(err, ErrStore) {
			t.Fatalf("empty: %v", err)
		}
		if err := store.Put(ctx, "x", Credentials{ClientID: "c", KeyID: "k", PrivateKeyPEM: secrets.New([]byte("junk"))}); !errors.Is(err, ErrKey) {
			t.Fatalf("junk key: %v", err)
		}
		if _, err := ConfigFrom(ctx, nil, "x"); !errors.Is(err, ErrStore) {
			t.Fatalf("nil store: %v", err)
		}
		if _, err := ConfigFrom(ctx, store, "missing"); !errors.Is(err, secrets.ErrNotFound) {
		...
		otherStore, _ := NewSealedStore(other)
		if err := otherStore.Put(ctx, "x", Credentials{ClientID: "c", KeyID: "k", PrivateKeyPEM: secrets.New(sec1PEM(t, newKey(t)))}); err != nil {
			t.Fatal(err)
		}
		store.mu.Lock()
		store.data["x"] = otherStore.Sealed("x")
		store.mu.Unlock()
		if _, err := store.Get(ctx, "x"); !errors.Is(err, ErrStore) || !errors.Is(err, crypt.ErrTampered) {
		...
		store.mu.Unlock()
		if _, err := store.Get(ctx, "g"); !errors.Is(err, ErrStore) {
		...
		store.mu.Unlock()
		if _, err := ConfigFrom(ctx, store, "b"); !errors.Is(err, ErrKey) {
