...
// maxMainLines bounds cmd/dmctl/main.go. The point is not the exact number
...
// stays a single function that parses argv and calls dmctl.Run; this is the
...
func TestCmdMainStaysThin(t *testing.T) {
	path := filepath.Join("..", "..", "cmd", "dmctl", "main.go")
	...
	if len(funcs) != 1 || funcs[0] != "main" {
		t.Fatalf("cmd/dmctl/main.go declares %v; it must declare only main, so the logic stays in internal/dmctl where the coverage gate reaches it", funcs)
	}

	if n := fset.File(file.Pos()).LineCount(); n > maxMainLines {
		t.Fatalf("cmd/dmctl/main.go is %d lines, over the %d-line budget: move the logic into internal/dmctl", n, maxMainLines)
	}
	...
	for _, decl := range file.Decls {
		...
		if gen.Tok == token.TYPE || gen.Tok == token.VAR {
			t.Errorf("cmd/dmctl/main.go declares a %s; it belongs in internal/dmctl", gen.Tok)
		}
	}
}

// The same argument applies to the server binary: cmd/dmserver is wiring,
// and internal/app is where the gate reaches.
func TestCmdMdmserverStaysThin(t *testing.T) {
	path := filepath.Join("..", "..", "cmd", "dmserver", "main.go")
	src, err := os.ReadFile(path) // #nosec G304 -- a fixed path inside the repository
	if err != nil {
		t.Fatal(err)
	}
	...
	// dmserver legitimately owns the listener and the signal handling, so
	// its budget is larger than dmctl's; what it must not grow is protocol
	// or admin logic.
	const maxServerLines = 200
	if n := fset.File(file.Pos()).LineCount(); n > maxServerLines {
		t.Fatalf("cmd/dmserver/main.go is %d lines, over the %d-line budget: move the logic into internal/app", n, maxServerLines)
	}
}
