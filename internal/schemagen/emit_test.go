...
func TestGenerateSynthetic(t *testing.T) {
	...
	if !strings.Contains(reg, `ErrorCodeWithCode = "com.example.code"`) ||
		strings.Contains(reg, "NoCode:") {
		t.Errorf("errors registry: %s", reg)
	}
	if !strings.Contains(
		get("checkin/registry.gen.go"),
		"NewResponse: func() any { return new(WithRespResponse) }",
	) {
		t.Error("checkin response constructor missing")
	}
	if !strings.Contains(get("ddm/registry.gen.go"), "KindCredential") ||
		strings.Contains(get("ddm/registry.gen.go"), `"DeclarationBase": {`) {
	...
	// doc.go is the package's only package comment and follows the
	// doc.go layout every hand-written package uses (see mdmprotocol/ddm/doc.go).
	doc := string(files["commands/doc.go"])
	if !strings.HasPrefix(
		...
		t.Errorf("doc.go header:\n%s", doc)
	}
	for _, want := range []string{
		"// # Design\n", "// # References\n", "devicemanagement/schema/EXPORTED_IDENTIFIERS.lock",
	...
	if strings.Count(doc, "// Package ") != 1 ||
		strings.Contains(string(files["commands/types.gen.go"]), "\n// Package commands") {
		t.Error("package comment must appear exactly once, in doc.go")
	}
}
