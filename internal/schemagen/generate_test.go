...
func TestGenerateWholeTree(t *testing.T) {
	...
	for name, data := range files {
		...
		if !strings.HasPrefix(string(data), "// Code generated by schemagen; DO NOT EDIT.") {
	...
	if !sort.StringsAreSorted(names) || len(names) < 1000 {
		t.Errorf(
			"EXPORTED_IDENTIFIERS.lock has %d entries, sorted=%v",
			len(names),
...
func TestWriteAndVerify(t *testing.T) {
	...
	keep := filepath.Join(out, "old", "doc.go")
	...
	for _, name := range []string{"gone.gen.go", "doc.gen.go", "conformance_gen_test.go", "doc.go"} {
		path := filepath.Join(out, "old", name)
		if err := os.WriteFile(
			path,
			[]byte("// Code generated by schemagen; DO NOT EDIT.\n\npackage old\n"),
			0o600,
		); err != nil {
			t.Fatal(err)
		}
		...
		if !errors.Is(err, ErrVerify) ||
			!strings.Contains(err.Error(), "old/"+name+": stale generated file") {
	...
	if err := os.WriteFile(
		lock,
		append(files["EXPORTED_IDENTIFIERS.lock"], []byte("commands/Obsolete\n")...),
		0o600,
	); err != nil {
		t.Fatal(err)
	}
	...
	if err := os.WriteFile(
		filepath.Join(out, "ALLOWED_REMOVALS.md"),
	...
	if err := os.Remove(filepath.Join(out, "commands", "doc.go")); err != nil {
		t.Fatal(err)
	}
	if err := Verify(root, out, Options{Commit: "test"}); !errors.Is(err, ErrVerify) {
	...
	if err := Write(filepath.Join(out, "GENERATED_FROM.json", "not-a-dir"), files); err == nil {
...
func TestReadGeneratedFrom(t *testing.T) {
	...
	p, err := ReadGeneratedFrom(
		filepath.Join("..", "..", "devicemanagement", "schema", "GENERATED_FROM.json"),
	)
	if err != nil {
...
func TestGenerateFormatFailureIsReported(t *testing.T) {
	...
	s := &Schema{
		Title:   "Bad",
		Path:    "mdm/commands/bad.yaml",
		Family:  FamilyCommands,
		Payload: Payload{RequestType: "Bad"},
	}
...
func TestSupportOverlayErrors(t *testing.T) {
	...
	st := &SchemaType{
		Schema: &Schema{
			Path:    "p",
			Payload: Payload{SupportedOS: SupportedOS{IOS: &OSSupport{Introduced: "bad"}}},
		},
	}
	...
	key := Key{
		Key:         "K",
		Type:        "<string>",
		SupportedOS: &SupportedOS{MacOS: &OSSupport{Removed: "bad"}},
	}
	st = &SchemaType{
		Name:    "T",
		Schema:  &Schema{Path: "p"},
		Request: &TypeDef{Fields: []*Field{{Name: "K", Key: "K", Path: "K", Src: &key}}},
	}
	if _, err := effective(st); err == nil {
