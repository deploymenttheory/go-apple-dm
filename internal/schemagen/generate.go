...
func removeStale(root *os.Root, dir string, written map[string]bool) error {
	...
	for _, d := range entries {
		...
		name := d.Name()
		generated, err := generatedFile(root, rel, name)
		if err != nil {
			return err
		}
		if !generated {
...
// doc.go shares its name with hand-written package documentation. Only our
// generated marker permits removing it when its schema family disappears.
func generatedFile(root *os.Root, path, name string) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("schemagen: read package documentation: %w", err)
	}
	return bytes.HasPrefix(data, []byte("// Code generated by schemagen; DO NOT EDIT.\n")), nil
}
...
func Verify(schemaRoot, outDir string, opts Options) error {
	...
	if err := fs.WalkDir(root.FS(), ".", func(path string, entry fs.DirEntry, walkErr error) error {
		...
		generated, err := generatedFile(root, path, entry.Name())
		if err != nil {
			return err
		}
		if generated {
